...
class InMemoryRepository implements IRepository {
  ...
  findConfigByName(_config_name: string): Promise<IConfigOutputDTO | null> {
    throw new Error('Method not implemented.');
  }
  readonly users: User[] = [];
  ...
  async findUserByEmail(
    ...
    const user =
      this.users.find((user) => user.getUserInfo().email! === email) || null;
  ...
  async createUser(user: User): Promise<ICreateUserOutputDTO> {
    const last_user = this.users.findLast(
      (user) => user.getUserInfo().user_id !== undefined
    );
  ...
  async findAllUsers(
    ...
    const all_users: ICreateUserOutputDTO[] = this.users
      .map((user) => {
        const user_info = user.getUserInfo();
      ...
      .filter(
        (user) => user.active === params.active || params.active === undefined
      );
  ...
  async createGroup(
    ...
    const user = this.users.find(
      (user) => user.getUserInfo().email === group.users_email
    );
    ...
    const last_group = this.groups.findLast(
      (group) => group.getMonitGroup().group_id! !== undefined
    );
  ...
  async findAllGroups(params: IQueryParams): Promise<IGroupOutputDTO[] | null> {
    const group_filtered = this.groups.filter(
      (group) =>
        group.getMonitGroup().active! === params.active ||
    ...
    for (let i = 0; i < group_filtered.length; i++) {
      ...
      let users_in_group = await this.findAllUserByGroupId(
        group.getMonitGroup().group_id!,
        params
      );
      ...
      all_groups.push({
        ...
        user: users_in_group.map((group_user) => {
          const user = this.users.find(
            (u) => u.getUserInfo().user_id === group_user!.user_id
          );
  ...
  async findGroupById(group_id: string): Promise<IGroupOutputDTO | null> {
    const group = this.groups.find(
      (group) => group.getMonitGroup().group_id! === group_id
    );
  ...
  async findGroupMembersById(
    ...
    const group_filtered = this.groups.filter(
      (group) =>
        (group.getMonitGroup().user_id! === user_id &&
          group.getMonitGroup()!.active === params.active) ||
        params.active === undefined
    );
  ...
  async findGroupByName(group_name: string): Promise<IGroupOutputDTO | null> {
    const groups = this.groups.find(
      (group) => group.getMonitGroup().group_name === group_name
    );
  ...
  async findAllJobs(
    ...
    const job_filtered = this.jobs.filter(
      (job) =>
        job.getJobInfo().active! === params.active ||
        params.active === undefined
    );
    ...
    for (let i = 0; i < job_filtered.length; i++) {
      ...
      let services_in_job = await this.findServicesByJobId(
        job_info.job_id!,
        params
      );
  ...
  async findAllJobsByGroupId(
    ...
    const job_filtered = this.jobs.filter(
      (job) =>
        (job.getJobInfo().group_id === group_id &&
          job.getJobInfo().active! === params.active) ||
        params.active === undefined
    );
  ...
  async findAllJobsWService(
    ...
    const filtered_job = all_jobs.filter(
      (all_job) => all_job.services.length >= 1
    );
  ...
  async findAllJobsWServiceByGroup(
    group_id: string,
    params: IQueryParams
  ): Promise<IJobOutputWServiceAvailableDTO[] | null> {
    const job_filtered = this.jobs
      ...
      .filter((job) => job.getJobInfo().group_id === group_id);
  ...
  async createService(
    ...
    const createJobOutput: IServiceOutputDTO = {
      ...
      last_run: new_service_info.last_run ?? null,
  ...
  async findServiceById(service_id: string): Promise<IServiceOutputDTO | null> {
    const service = this.services.find(
      (service) => service.getServiceInfo().service_id! === service_id
    );
    ...
    const service_output: IServiceOutputDTO = {
      ...
      last_run: service_info.last_run ?? null,
  ...
  async findServiceByName(
    ...
    const service = this.services.find(
      (service) => service.getServiceInfo().service_name! === service_name
    );
  ...
  async findServicesByGroupId(
    ...
    const services = this.services.filter(
      (service) =>
        (service.getServiceInfo().group_id! === group_id &&
          service.getServiceInfo().active! === params.active) ||
        params.active === undefined
    );
  ...
  async findServicesByJobId(
    ...
    const services = this.services.filter(
      (service) =>
        (service.getServiceInfo().job_id! === job_id &&
          service.getServiceInfo().active! === params.active) ||
        params.active === undefined
    );
  ...
  async findAllServices(
    ...
    const service_filtered = this.services.filter(
      (service) =>
        service.getServiceInfo()!.active === params.active ||
        params.active === undefined
    );
  ...
  async createServiceLog(
    ...
    const last_service_id = this.service_logs.findLast(
      (job) => job.getServiceLogInfo().service_log_id !== undefined
    );
    const new_id = last_service_id
      ? last_service_id.getServiceLogInfo().service_log_id + 'a'
      : 'a';

    const service_payload = {
      service_log_id: new_id,
      ...service_log_payload,
      method: service_log_payload.method ?? 'JOB',
    };
  ...
  async findAllServicesLogByJobName(
    ...
    for (let i = 0; i < services.length; i++) {
      ...
      const service_logs = this.service_logs.filter(
        (service_log) =>
          service_log.getServiceLogInfo().service_id === service.service_id
      );
  ...
  async findServicesLogByServiceId(
    ...
    const service_logs = this.service_logs.filter(
      (service_log) => service_log.getServiceLogInfo().service_id === service_id
    );
    let all_service_logs: IServiceLogOutputDTO[] = [];
    for (let j = 0; j < service_logs.length; j++) {
      const service_log = service_logs[j];
  ...
  async createJobLog(job_log_payload: IJobLogInputDTO): Promise<void> {
    const last_job_id = this.job_logs.findLast(
      (job) => job.getJobLogInfo().job_log_id !== undefined
    );
    const new_id = last_job_id
      ? last_job_id.getJobLogInfo().job_log_id + 'a'
      : 'a';
  ...
  async findAllJobsLogByJobId(
    ...
    const all_jobs = this.job_logs.filter(
      (job_log) => job_log.getJobLogInfo().job_id === job_id
    );
  ...
  async findAllJobsLogByGroupId(
    ...
    for (let i = 0; i < jobsByGroup.length; i++) {
      ...
      const job_logs_by_job_id = await this.findAllJobsLogByJobId(
        jobGroup.job_id,
        params
      );
