...
class CreateJobUseCase {
    ...
    async execute(job_payload: IJobInputDTO, data_in_token: IDataInToken) {
        ...
        const groups = await this.repository.findGroupByName(job_payload.group_name)
        ...
        const alreadyExistJob = await this.repository.findJobByName(job_payload.job_name)

        if (alreadyExistJob) {
            throw new Error("This job name is already in use!");
        }

        const response = await this.repository.createJob(job_payload, data_in_token.user_id)
