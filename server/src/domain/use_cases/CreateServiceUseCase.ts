...
class CreateServiceUseCase {
    ...
    async execute(service_payload: IServiceInputDTO, data_in_token: IDataInToken) {
        ...
        const alreadyExistService = await this.dbRepository.findServiceByName(service_payload.service_name);

        if (alreadyExistService) {
            throw new Error("Already exist a service with the same name.");
        }

        const response = await this.dbRepository.createService(service_payload, data_in_token.user_id);
