...
class CreateUserUseCase {
  constructor(
    private dbRepository: IRepository,
    private hashService: IHashPassword
  ) {}

  async execute(
    user_payload: ICreateUserInputDTO
    ...
    const already_exist_user = await this.dbRepository.findUserByEmail(
      user_info.email
    );
    ...
    const response = await this.dbRepository.createUser(
      new User(userhashedPwd)
    );
